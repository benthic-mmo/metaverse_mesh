...
use metaverse_messages::utils::render_data::AvatarObject;
...
pub fn bake_avatar(avatar: AvatarObject, path: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for object in avatar.objects {
        let json_str = fs::read_to_string(&object).expect(&format!("Failed to read {:?}", object));
        let parts: Vec<RenderObject> = serde_json::from_str(&json_str).unwrap();
        for part in parts {
            let vertices_transformed: Vec<Vec3> = part.vertices.clone();
            ...
            let positions_accessor = root.push(Accessor {
                ...
                byte_offset: Some(USize64(0)),
                count: USize64::from(part.vertices.len()),
                component_type: Valid(GenericComponentType(ComponentType::F32)),
                type_: Valid(gltf_json::accessor::Type::Vec3),
                min: Some(Value::from(Vec::from(min))),
                max: Some(Value::from(Vec::from(max))),
            ...
            let mut index_bytes = Vec::with_capacity(part.indices.len() * 2);
            ...
            let index_accessor = root.push(Accessor {
                ...
                byte_offset: Some(USize64(0)),
                count: USize64::from(part.indices.len()),
                component_type: Valid(GenericComponentType(ComponentType::U16)),
                type_: Valid(gltf_json::accessor::Type::Scalar),
                normalized: false,
                sparse: None,
            ...
            for vw in &part.skin.as_ref().unwrap().weights {
            ...
            let joint_indices_accessor = root.push(Accessor {
                ...
                byte_offset: Some(USize64(0)),
                count: USize64::from(part.skin.as_ref().unwrap().weights.len()),
            ...
            combined_weights.push(part.skin.unwrap().weights);
        }
    }
    ...
    for joint_name in &bones {
        if let Some(joint) = avatar.global_skeleton.joints.get(joint_name) {
            ...
            if avatar.global_skeleton.root.contains(joint_name) {
                nodes.push(node_index)
            }
    ...
    for joint_name in &bones {
        let joint = avatar.global_skeleton.joints.get(joint_name).unwrap();
